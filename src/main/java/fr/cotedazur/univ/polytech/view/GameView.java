...
public class GameView {
    ...
    public void printPlayerAction(String action, Player player) {
        String playerString = "Le joueur " + player.getName();
        if (action.equals("2golds"))
            System.out.println(playerString + " a choisis de prendre 2 pièces d'or, il possède maintenant " + player.getGolds() + " pièces d'or.");
        else if (action.equals("drawCard"))
            System.out.println(playerString + " a choisis de piocher une carte, il possède maintenant " + player.getHands().size() + " cartes dans sa main");
        if (action.equals("putDistrict"))
            System.out.println(playerString + " a choisis de placer " + player.getBoard().get(player.getBoard().size() - 1));
    }
